    ...
    async def _ensure_ea_service_initialized(self):
        ...
        if EA_SERVICES_AVAILABLE and EAKnowledgeGraph:
            try:
                ...
                self.ea_graph_service = await loop.run_in_executor(
                    None,
                    lambda: EAKnowledgeGraph(db)
                )
                
                if EAHybridQuery:
                    try:
                        from metadata_service.src.services.ea_vectorization_service import EAVectorizationService
                        ea_vector_service = await loop.run_in_executor(
                            None,
                            lambda: EAVectorizationService(db)
                        )
                        self.ea_hybrid_query = await loop.run_in_executor(
                            None,
                            lambda: EAHybridQuery(db, self.ea_graph_service, ea_vector_service)
                        )
                    except Exception as e:
