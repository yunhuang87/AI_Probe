...
class ConversationAgent:
    ...
    async def understand_conversation(
        ...
        Returns:
            意图分析结果
        """
        llm_available = self.use_llm and deepseek_llm.llm
        ...
        # 优先使用LLM进行意图识别（符合“先LLM再路由”的要求）
        if prompt_engine and prompt_context and llm_available:
            try:
        ...
        if self.use_metadata_first and self.metadata_first_recognizer:
            try:
                logger.debug("LLM unavailable, using metadata-first intent recognition")
                ...
                logger.warning(f"Metadata-first recognition failed: {e}, falling back to rules")
