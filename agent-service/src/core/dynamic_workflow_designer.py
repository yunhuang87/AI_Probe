    ...
    async def _design_with_streaming(
        ...
        try:
            ...
            try:
                ...
                # 构建对话历史部分（避免在f-string中使用反斜杠）
                history_section = ""
                if conversation_history_text:
                    history_section = f"对话历史（最近10条）:\n{conversation_history_text}\n"
                
                thinking_prompt = f"""
用户请求: "{user_input}"
{history_section}用户上下文: {json.dumps(context, ensure_ascii=False, indent=2)}
...
- **思考过程要简洁明了，控制在800-1200字以内**，重点突出核心意图识别和执行方案
- 避免过度展开细节，直接说明会使用哪些智能体和资源，以及为什么选择它们
...
"""
    ...
    async def _analyze_request(
        ...
        # 构建对话历史和资源上下文部分（避免在f-string中使用反斜杠）
        history_section = ""
        if conversation_history_text:
            history_section = f"对话历史（最近10条）:\n{conversation_history_text}\n"
        ...
        resource_section = ""
        if resource_context:
            resource_section = f"资源上下文: {json.dumps(resource_context, ensure_ascii=False, indent=2)}\n"
