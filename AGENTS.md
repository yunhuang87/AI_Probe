...
### 本地部署（推荐：前端宿主机 + 后端 Docker）

- **前端**：在宿主机用 Node 直接运行，不启动 Docker 中的 web-ui。
  - 在项目根目录执行：`cd web-ui && npm install --legacy-peer-deps && npm run dev`
  - 访问：`http://localhost:3000`；登录页使用 API 网关 `http://localhost:8080/api/auth/login`（见 `web-ui/.env.local`）。
- **后端**：仅启动 Docker 中的后端服务（默认不包含带 `profiles: frontend` 的 web-ui）。
  - 启动：`docker compose up -d postgres redis api-gateway auth-service chat-service workflow-engine mcp-gateway agent-service agent-orchestrator agent-registry registry-service config-center memory-service dag-orchestrator minio neo4j qdrant`（若某服务镜像不存在，可去掉该服务或先 `docker compose build <服务名>`）。
  - 需要知识库/元数据服务时：`docker compose build knowledge-base metadata-service && docker compose up -d`。
- **环境**：`web-ui/.env.local` 中已配置 `NEXT_PUBLIC_API_GATEWAY_URL=http://localhost:8080` 等，前端通过 8080 访问后端。
