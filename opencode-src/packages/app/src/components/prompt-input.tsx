...
const INPUT_SPLIT_MAX = 2000
const INPUT_SPLIT_LINES = 300
const OUTPUT_LIMIT_MAX = 1200
  ...
  const handleSubmit = async (event: Event) => {
    ...
    const splitText = (input: string) => {
      ...
      for (const line of lines) {
        if (line.length > INPUT_SPLIT_MAX) {
          flush()
          let remaining = line
          while (remaining.length > INPUT_SPLIT_MAX) {
            segments.push(remaining.slice(0, INPUT_SPLIT_MAX))
            remaining = remaining.slice(INPUT_SPLIT_MAX)
          }
          current = remaining
          currentLines = remaining.length ? 1 : 0
          continue
        }

        const next = current.length ? `${current}\n${line}` : line
        const nextLines = current.length ? currentLines + 1 : 1
        if (next.length > INPUT_SPLIT_MAX || nextLines > INPUT_SPLIT_LINES) {
          flush()
          current = line
          currentLines = 1
          continue
    ...
    const limitSystem = `请将回复控制在${OUTPUT_LIMIT_MAX}字以内；若需要继续，请仅回复“继续”。`
    const textSegments = splitText(text)
    const hasMultipleSegments = textSegments.length > 1
    const entries = textSegments.map((segment, index) => {
      ...
      const parts = isFirst
        ? [textPart, ...fileAttachmentParts, ...contextParts, ...agentAttachmentParts, ...imageAttachmentParts]
        : [textPart]
      const optimisticParts = parts.map((part) => ({
        ...part,
        sessionID: session.id,
        messageID,
      })) as unknown as Part[]
      ...
      return {
        ...
        optimisticMessage,
        noReply: !isLast,
        system: isLast ? limitSystem : undefined,
      }
    })

    const pendingMessages = new Set<string>()

    const addOptimisticMessage = (entry: {
      messageID: string
      optimisticMessage: Message
      optimisticParts: Part[]
    }) => {
      const messageID = entry.messageID
      const optimisticMessage = entry.optimisticMessage
      const optimisticParts = entry.optimisticParts
      if (sessionDirectory === projectDirectory) {
        sync.set(
          produce((draft) => {
    ...
    for (const entry of entries) {
      pendingMessages.add(entry.messageID)
      addOptimisticMessage(entry)
    }
    if (hasMultipleSegments) {
      showToast({
        title: "输入已拆分",
        description: `已拆分为 ${entries.length} 段，将按顺序发送。`,
      })
    }
