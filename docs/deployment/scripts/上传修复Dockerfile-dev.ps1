# PowerShell Script - Upload fixed Dockerfile.dev to server
$keyPath = "E:\enterprise-ai-platform\enterprise_ai_platform.pem"
$server = "ubuntu@43.143.139.197"

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Uploading fixed Dockerfile.dev to server" -ForegroundColor Cyan
Write-Host "Added database dependencies installation" -ForegroundColor Yellow
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

Write-Host "Uploading mcp-gateway/Dockerfile.dev..." -ForegroundColor Yellow
$result = scp -i $keyPath -o StrictHostKeyChecking=no "mcp-gateway/Dockerfile.dev" "${server}:/opt/enterprise-ai-platform/mcp-gateway/" 2>&1

if ($LASTEXITCODE -eq 0) {
    Write-Host "[OK] Dockerfile.dev uploaded successfully!" -ForegroundColor Green
    Write-Host ""
