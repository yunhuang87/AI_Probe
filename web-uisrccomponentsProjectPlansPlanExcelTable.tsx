...
import type { Column, Row, CellClickArgs, CellKeyDownArgs, SortColumn } from 'react-data-grid';

// 动态导入DataGrid以避免SSR问题
const DataGrid = dynamic(() => import('react-data-grid').then((mod) => mod.default), {
  ssr: false,
...
export default function PlanExcelTable({
  ...
  // 定义列
  const columns: Column<PlanExcelRow>[] = useMemo(
    () => [
      {
        key: 'name',
        name: '名称',
  ...
  // 处理单元格点击
  const handleCellClick = useCallback((args: CellClickArgs<PlanExcelRow>) => {
    // 可以在这里添加点击逻辑
  }, []);

  // 处理键盘事件
  const handleCellKeyDown = useCallback((args: CellKeyDownArgs<PlanExcelRow>) => {
    ...
    <div className="w-full">
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <DataGrid
          ...
          style={{ height: '600px', minHeight: 400 }}
