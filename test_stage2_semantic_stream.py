...
def test_semantic_stream(url: str, user_input: str, user_id: Optional[str] = None, session_id: Optional[str] = None):
    """测试语义引擎流式查询功能"""
    print(f"\n{'='*60}")
    print(f"阶段2测试：语义引擎流式查询")
    print(f"{'='*60}")
    print(f"URL: {url}")
                ...
                try:
                    ...
                    if stage == "semantic_search":
                        ...
                        if semantic_chunk:
                            ...
                            if semantic_chunk.get('activities_count'):
                                print(f"    活动数量: {semantic_chunk.get('activities_count')}")
                            if semantic_chunk.get('activities_batch'):
                                batch = semantic_chunk.get('activities_batch', [])
                                print(f"    活动批次: {len(batch)} 个活动")
                                for i, act in enumerate(batch[:2], 1):  # 只显示前2个
                                    print(f"      {i}. {act.get('name', 'N/A')} (相似度: {act.get('similarity_score', 0):.2f})")
                    ...
                    if stage in ['complete', 'error']:
                        ...
                        if "semantic_search" in stages_seen:
                            print(f"\n[SUCCESS] 阶段2功能验证成功！")
                            print(f"  - 语义引擎流式查询已集成")
                            print(f"  - 收到 {len([c for c in semantic_chunks if c.get('stage') == 'semantic_search'])} 个语义引擎进度更新")
                        else:
                            print(f"\n[WARN] 未检测到语义引擎流式查询（可能因为语义引擎不可用）")
