"""
在Docker容器内构建语义索引
这个脚本设计为在Docker容器内运行，使用容器服务名访问其他服务
"""
import asyncio
import sys
import os
...
# 配置参数
DEFAULT_BATCH_SIZE = int(os.getenv("SEMANTIC_BATCH_SIZE", "50"))
MAX_RETRIES = 3
RETRY_DELAY = 2
BATCH_DELAY = 1
PROGRESS_FILE = "/app/scripts/semantic_build_progress.json"  # 容器内路径（与脚本同目录）
...
async def build_semantic_in_docker(
    ...
    # 在容器内使用服务名，否则使用localhost
    if in_container:
        metadata_service_url = os.getenv("METADATA_SERVICE_URL", "http://metadata-service:8005")
        knowledge_base_url = os.getenv("KNOWLEDGE_BASE_URL", "http://knowledge-base:8004")
        print("=" * 80)
        print("在Docker容器内构建语义索引")
        print("=" * 80)
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="在Docker容器内构建SAP元数据语义分析索引",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 在容器内运行（使用docker exec）
  docker exec -it enterprise-ai-sap-metadata-agent python build_semantic_in_docker.py
  
  # 指定批次大小
  docker exec -it enterprise-ai-sap-metadata-agent python build_semantic_in_docker.py --batch-size 100
  
  # 从进度文件恢复
  docker exec -it enterprise-ai-sap-metadata-agent python build_semantic_in_docker.py --resume
        """
    )
    parser.add_argument(
        "--assets-offset",
