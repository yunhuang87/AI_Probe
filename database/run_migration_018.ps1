...
Write-Host "运行数据库迁移 018" -ForegroundColor Cyan
Write-Host "添加 knowledge_bases 表和 documents.knowledge_base_id 字段" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
# 运行迁移
Write-Host "运行迁移 018..." -ForegroundColor Yellow
try {
    # 设置环境变量
    ...
    # 运行Python脚本
    $pythonScript = Join-Path $scriptDir "run_migration_018.py"
    & $venvPython $pythonScript
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host ""
        Write-Host "✅ 迁移 018 执行成功！" -ForegroundColor Green
    } else {
        Write-Host ""
    ...
    Write-Host ""
    Write-Host "❌ 迁移执行出错: $_" -ForegroundColor Red
    exit 1
}
