    ...
    try {
      ...
      try {
        ...
        if (isUnauthorized) {
          ...
          // 其他错误（网络错误、超时等）- 有缓存则继续用缓存，避免因 /users/me 慢而清空用户
          if (!cachedUser) {
            console.error('Failed to get user info:', error);
            setUser(null);
            setLoading(false);
          }
          // 有缓存时保持显示，不打印为 error，仅便于排查时查看
          if (cachedUser && error?.message === 'Request timeout') {
            console.warn('Get user info timeout, using cached user');
          }
        }
      }
    } catch (error) {
