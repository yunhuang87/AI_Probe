...
export function useChat(options: UseChatOptions = {}) {
  ...
  const [workflowStatuses, setWorkflowStatuses] = useState<Map<string, WorkflowExecutionStatus>>(new Map())
  ...
  // 执行工作流
  const executeWorkflow = useCallback(async (workflowName: string, userId: string, username: string) => {
  ...
  // 执行工具
  const executeTool = useCallback(async (toolName: string, userId: string, username: string) => {
    if (!currentSessionId) return
  ...
  // 处理文本消息（可以集成AI回复）
  const handleTextMessage = useCallback(async (content: string, userId: string, username: string) => {
