...
export function DynamicWorkflowDisplay({ chunks, onComplete }: DynamicWorkflowDisplayProps) {
    ...
    for (const chunk of chunks) {
      if (chunk.type === 'thinking') {
        ...
        if (chunk.stage === 'execution_complete') {
          // 提取最终结果，支持多种格式
          result = chunk.final_result || chunk.result || chunk.data?.result || chunk.data?.final_result
          completed = true
          console.log('[DynamicWorkflowDisplay] Execution complete (from execution type), final_result:', result)
        }
        // 即使不是execution_complete，也检查是否有final_result（可能流被中断但结果已发送）
        if (!result && (chunk.final_result || chunk.result || chunk.data?.result || chunk.data?.final_result)) {
          result = chunk.final_result || chunk.result || chunk.data?.result || chunk.data?.final_result
          console.log('[DynamicWorkflowDisplay] Found final_result in execution chunk (stage:', chunk.stage, '):', result)
        }
        ...
        result = chunk.final_result || chunk.result || chunk.data?.result || chunk.data?.final_result
        completed = true
        console.log('[DynamicWorkflowDisplay] Execution complete (from execution_complete type), final_result:', result)
        console.log('[DynamicWorkflowDisplay] Full chunk:', JSON.stringify(chunk, null, 2).substring(0, 500))
      }
    }
    ...
    if (!completed && execution.length > 0 && !result) {
      ...
      if (lastExecutionChunk.agent_result?.output) {
        ...
        if (typeof agentOutput === 'object' && agentOutput !== null) {
          result = {
            formatted_output: agentOutput.formatted_output || agentOutput.formatted_text || JSON.stringify(agentOutput, null, 2),
            output: agentOutput
          }
          completed = true
          ...
          completed = true
          console.log('[DynamicWorkflowDisplay] Extracted result from last agent_result (string):', result)
        }
      }
    }
              ...
              {thinkingProcess.length > 0 && (() => {
                ...
                if (!fullThinkingMessage) {
                  ...
                  if (allMessages.length > 0) {
                    // 使用最长的message
                    fullThinkingMessage = allMessages.reduce((longest, current) => 
                      ...
                      <div className="thinking-text bg-white dark:bg-gray-800 rounded-lg p-4 border-l-4 border-purple-500 dark:border-purple-500">
                        ...
                        <div className="text-xs text-gray-700 dark:text-gray-300 space-y-2">
                          {lastChunk.design.agents && (
                            <div>
                              <span className="font-semibold">智能体数量:</span> {lastChunk.design.agents.length}
                            </div>
                          )}
                          {lastChunk.design.execution_layers && (
                            <div>
                              <span className="font-semibold">执行层数:</span> {lastChunk.design.execution_layers.length}
                            </div>
                          )}
                          {lastChunk.design.complexity && (
                            <div>
                              <span className="font-semibold">复杂度:</span> {lastChunk.design.complexity}
                            </div>
                          )}
                ...
                if (fullThinkingMessage) {
                  ...
                  if (originalLength > 100 && cleanedLength < originalLength * 0.7) {
                    console.warn('[DynamicWorkflowDisplay] ⚠️ Thinking message cleaned too aggressively, using original content')
                    cleanMessage = fullThinkingMessage // 使用原始完整内容
                  }
                  if (originalLength > 100 && cleanedLength < originalLength * 0.8) {
                    // 如果清理后内容减少了超过20%，记录警告
                    console.warn('[DynamicWorkflowDisplay] ⚠️ Thinking message cleaned significantly:', {
                      originalLength,
                      cleanedLength,
                      reduction: ((originalLength - cleanedLength) / originalLength * 100).toFixed(1) + '%',
                  ...
                  if (!cleanMessage || cleanMessage.length < 3) {
                    console.warn('[DynamicWorkflowDisplay] ⚠️ Thinking message became empty after cleaning, using original')
                    cleanMessage = fullThinkingMessage // 如果清理后为空，使用原始内容
                  }
                  
                  return (
                    <div key="thinking-last" className="thinking-text bg-white/70 dark:bg-gray-800/70 rounded-lg p-4 border-l-4 border-blue-400 dark:border-blue-500 shadow-sm">
                      <div className="text-sm text-gray-800 dark:text-gray-200 leading-relaxed whitespace-pre-wrap font-medium">
                        {cleanMessage}
                      </div>
                    </div>
                          ...
                          {typeof chunk.network_summary === 'string' ? (
                            ...
                            <>
                              {chunk.network_summary.total_agents && (
                                <div><span className="font-semibold">智能体总数:</span> {chunk.network_summary.total_agents}</div>
                              )}
                              {chunk.network_summary.total_layers && (
                                <div><span className="font-semibold">执行层数:</span> {chunk.network_summary.total_layers}</div>
                              )}
                              {chunk.network_summary.complexity && (
                                <div><span className="font-semibold">复杂度:</span> {chunk.network_summary.complexity}</div>
                              )}
                              <details className="mt-2">
                  ...
                  <div className="ml-4 border-l-2 border-green-300 dark:border-green-600 pl-3">
                    <div className="text-green-600 dark:text-green-400 font-medium flex items-center justify-between">
                      <div className="flex items-center">
                        <LayerIcon className="mr-2 flex-shrink-0" size={16} />
                        第 {chunk.layer_number}/{chunk.total_layers} 层
                      </div>
                      {chunk.progress !== undefined && (
                        <span className="text-xs opacity-75">{chunk.progress}%</span>
                      )}
                ...
                {chunk.stage === 'agent_start' && (
                  <div className="ml-8 border-l-2 border-green-200 dark:border-green-700 pl-3">
                    <div className="flex items-center text-green-600 dark:text-green-400">
                      ...
                      <span className="text-xs flex-1">{chunk.agent_task || chunk.agent_type || chunk.agent_id}</span>
                      {chunk.progress !== undefined && (
                        <div className="ml-2 flex items-center gap-1.5">
                          ...
                          <span className="text-xs opacity-75 min-w-[2.5rem]">{chunk.progress}%</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="ml-8 border-l-2 border-green-200 dark:border-green-700 pl-3">
                    <div className="flex items-center justify-between text-green-600 dark:text-green-400">
                      ...
                      <div className="flex items-center gap-2 text-xs opacity-75">
                        {chunk.agent_result.execution_time != null && typeof chunk.agent_result.execution_time === 'number' && (
                          <span>{chunk.agent_result.execution_time.toFixed(2)}s</span>
                        )}
                        {chunk.agent_result.confidence != null && typeof chunk.agent_result.confidence === 'number' && (
                        ...
                        {chunk.agent_result.quality_score != null && typeof chunk.agent_result.quality_score === 'number' && (
                    ...
                    {chunk.agent_result.error && (
                      <div className="mt-2 p-2 bg-red-50/50 dark:bg-red-900/20 rounded text-xs text-red-700 dark:text-red-300 border-l-2 border-red-400">
                        ...
                        {chunk.agent_result.error_code && (
                          <div className="mt-1 opacity-75">错误代码: {chunk.agent_result.error_code}</div>
                        )}
                      </div>
                    )}
                    
                    {/* 显示警告信息 */}
                    {chunk.agent_result.warnings && Array.isArray(chunk.agent_result.warnings) && chunk.agent_result.warnings.length > 0 && (
                    ...
                    {/* 显示建议 */}
                    {chunk.agent_result.recommendations && Array.isArray(chunk.agent_result.recommendations) && chunk.agent_result.recommendations.length > 0 && (
                      <div className="mt-2 p-2 bg-blue-50/50 dark:bg-blue-900/20 rounded text-xs text-blue-700 dark:text-blue-300 border-l-2 border-blue-400">
                        ...
                        <ul className="list-disc list-inside space-y-1">
                          {chunk.agent_result.recommendations.map((rec: string, idx: number) => (
                            <li key={idx}>{rec}</li>
                          ))}
                        </ul>
                      </div>
                    ...
                    {/* 显示下一步操作 */}
                    {chunk.agent_result.next_actions && Array.isArray(chunk.agent_result.next_actions) && chunk.agent_result.next_actions.length > 0 && (
                      <div className="mt-2 p-2 bg-green-50/50 dark:bg-green-900/20 rounded text-xs text-green-700 dark:text-green-300 border-l-2 border-green-400">
                        ...
                        <ul className="list-disc list-inside space-y-1">
                          {chunk.agent_result.next_actions.map((action: string, idx: number) => (
                    ...
                    {/* 显示产物（artifacts） */}
                    {chunk.agent_result.artifacts && Array.isArray(chunk.agent_result.artifacts) && chunk.agent_result.artifacts.length > 0 && (
                      <div className="mt-2 p-2 bg-indigo-50/50 dark:bg-indigo-900/20 rounded text-xs text-indigo-700 dark:text-indigo-300 border-l-2 border-indigo-400">
                        <div className="font-semibold mb-1">📎 产物 ({chunk.agent_result.artifacts.length}):</div>
                        <div className="space-y-1">
                          {chunk.agent_result.artifacts.map((artifact: any, idx: number) => (
                            <div key={idx} className="flex items-start gap-2">
                              <span className="font-medium">{artifact.name || artifact.file_name || `产物 ${idx + 1}`}:</span>
                              <span className="opacity-75">{artifact.type || artifact.mime_type || '未知类型'}</span>
                              {artifact.url && (
                                <a href={artifact.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">
                                  查看
                                </a>
                              )}
                            </div>
                      ...
                      <div className="mt-2 p-2 bg-gray-50/50 dark:bg-gray-800/50 rounded text-xs text-gray-600 dark:text-gray-400 border-l-2 border-gray-400">
                        ...
                        <div className="space-y-1">
                          {chunk.agent_result.execution_metadata.tokens_used && (
                            <div>Token使用: {chunk.agent_result.execution_metadata.tokens_used}</div>
                          )}
                          ...
                          {chunk.agent_result.execution_metadata.tools_used && Array.isArray(chunk.agent_result.execution_metadata.tools_used) && (
                            <div>工具: {chunk.agent_result.execution_metadata.tools_used.join(', ')}</div>
                          )}
                        </div>
                      </div>
                    )}
          ...
          <div className="text-sm text-yellow-800 dark:text-yellow-300">
            ...
            <details className="mt-2">
              ...
              <pre className="mt-2 text-xs overflow-auto max-h-40">
                {JSON.stringify(chunks.filter(c => c.type === 'execution_complete'), null, 2)}
              </pre>
            </details>
          </div>
        </div>
        ...
        <div className="final-result mt-8 bg-gradient-to-br from-white via-blue-50/50 to-indigo-50/30 dark:from-gray-800 dark:via-gray-800 dark:to-gray-900 rounded-2xl p-8 border-2 border-blue-200/50 dark:border-gray-700 shadow-2xl backdrop-blur-sm relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-8 pb-6 border-b-2 border-blue-200/50 dark:border-gray-700 relative z-10">
            ...
            {finalResult.execution_summary && (
              <div className="text-xs text-gray-500 dark:text-gray-400 bg-white/60 dark:bg-gray-700/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-gray-600">
                {finalResult.execution_summary.executed_agents}/{finalResult.execution_summary.total_agents} 智能体 | 
                成功率: {finalResult.execution_summary.success_rate != null && typeof finalResult.execution_summary.success_rate === 'number' 
          ...
          {finalResult.formatted_output && (
            <div className="final-output-content bg-white dark:bg-gray-800 rounded-lg p-6 md:p-8 border border-gray-200 dark:border-gray-700 animate-fade-in relative overflow-hidden">
              ...
              <div className="prose prose-lg dark:prose-invert max-w-none relative z-10 prose-headings:mt-4 prose-headings:mb-3 prose-p:my-3 prose-ul:my-3 prose-ol:my-3 prose-table:my-4">
                <MessageContent 
                  content={finalResult.formatted_output} 
                  isOwnMessage={false}
                />
              </div>
            </div>
          ...
          {!finalResult.formatted_output && finalResult.synthesized_result && (
            <div className="space-y-4">
              {/* 主要发现 */}
              {finalResult.synthesized_result.key_findings && finalResult.synthesized_result.key_findings.length > 0 && (
                <div className="bg-amber-50/50 dark:bg-amber-900/20 rounded-lg p-4 border-l-4 border-amber-400">
                  <div className="text-sm font-semibold text-amber-800 dark:text-amber-300 mb-2">主要发现</div>
                  <ul className="space-y-2 text-sm text-gray-700 dark:text-gray-300">
                    {finalResult.synthesized_result.key_findings.map((finding: string, idx: number) => (
              ...
              {/* 建议 */}
              {finalResult.synthesized_result.recommendations && finalResult.synthesized_result.recommendations.length > 0 && (
                <div className="bg-green-50/50 dark:bg-green-900/20 rounded-lg p-4 border-l-4 border-green-400">
                  <div className="text-sm font-semibold text-green-800 dark:text-green-300 mb-2">建议与后续步骤</div>
                  <ul className="space-y-2 text-sm text-gray-700 dark:text-gray-300">
                    {finalResult.synthesized_result.recommendations.map((rec: string, idx: number) => (
                      <li key={idx} className="flex items-start">
