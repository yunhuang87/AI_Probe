...
export function MessageContent({ content, isOwnMessage = false }: MessageContentProps) {
  const formattedContent = useMemo(() => {
    ...
    // 如果内容看起来像 result_synthesis 的 Python dict 字符串，优先提取格式化结果
    if (
      formatted.includes('agent_type') &&
      (formatted.includes('formatted_output') || formatted.includes('main_content'))
    ) {
      const extracted =
        extractFieldFromString(formatted, 'formatted_output') ||
        extractFieldFromString(formatted, 'main_content') ||
        extractFieldFromString(formatted, 'summary');
      if (extracted) {
        formatted = extracted;
      }
    }
    ...
    formatted = formatted.replace(
      /✅/g,
      '<span class="status-icon status-success"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></span>'
    );
    formatted = formatted.replace(
      /❌/g,
      '<span class="status-icon status-error"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg></span>'
    );
    formatted = formatted.replace(
      /⚠️/g,
      '<span class="status-icon status-warning"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg></span>'
    );
    formatted = formatted.replace(
      /📊/g,
      '<span class="status-icon status-info"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3v18h18"/><path d="m19 9-5 5-4-4-3 3"/></svg></span>'
    );
    formatted = formatted.replace(
      /🔍/g,
      '<span class="status-icon status-info"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg></span>'
    );
    formatted = formatted.replace(
      /🛠️/g,
      '<span class="status-icon status-info"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg></span>'
    );
    formatted = formatted.replace(
      /📋/g,
      '<span class="status-icon status-info"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M16 13H8"/><path d="M16 17H8"/><path d="M10 9H8"/></svg></span>'
    );
    formatted = formatted.replace(
      /🚀/g,
      '<span class="status-icon status-success"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91-.09z"/><path d="m12 15-3-3a22 22 0 0 1 2-3.95A12.88 12.88 0 0 1 22 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 0 1-4 2z"/><path d="M9 12H4s.55-3.03 2-4c1.62-1.08 5 0 5 0"/><path d="M12 15v5s3.03-.55 4-2c1.08-1.62 0-5 0-5"/></svg></span>'
    );
    formatted = formatted.replace(
      /📈/g,
      '<span class="status-icon status-success"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg></span>'
    );
    formatted = formatted.replace(
      /💡/g,
      '<span class="status-icon status-info"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" x2="12" y1="2" y2="3"/><line x1="12" x2="12" y1="21" y2="22"/><line x1="4.22" x2="5.64" y1="4.22" y2="5.64"/><line x1="18.36" x2="19.78" y1="18.36" y2="19.78"/><line x1="2" x2="3" y1="12" y2="12"/><line x1="21" x2="22" y1="12" y2="12"/><line x1="4.22" x2="5.64" y1="19.78" y2="18.36"/><line x1="18.36" x2="19.78" y1="5.64" y2="4.22"/><circle cx="12" cy="12" r="5"/></svg></span>'
    );
    formatted = formatted.replace(
      /🎯/g,
      '<span class="status-icon status-success"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg></span>'
    );
    formatted = formatted.replace(
      /📚/g,
      '<span class="status-icon status-info"><svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"/></svg></span>'
    );
    formatted = formatted.replace(
      /🏢/g,
    ...
    formatted = formatted.replace(
      /(任务状态|执行状态|状态|成功智能体|核心问题)[：:]\s*([^\n<]+)/g,
      (match, label, status) => {
        const trimmedStatus = status.trim();
        const statusClass =
          trimmedStatus.includes('✅') || trimmedStatus.includes('成功')
            ? 'status-success'
            : trimmedStatus.includes('❌') || trimmedStatus.includes('失败')
              ? 'status-error'
              : trimmedStatus.includes('⚠️') || trimmedStatus.includes('警告')
                ? 'status-warning'
                : 'status-info';
    ...
    formatted = formatted.replace(
      /(总体状态|失败原因|恢复建议|执行概况|关键洞察|改进建议)[：:]\s*([^\n<]+)/g,
      (match, label, value) => {
        const trimmedValue = value.trim();
        const valueClass =
          trimmedValue.includes('✅') || trimmedValue.includes('成功')
            ? 'status-success'
            : trimmedValue.includes('❌') || trimmedValue.includes('失败')
              ? 'status-error'
              : trimmedValue.includes('⚠️') || trimmedValue.includes('警告')
                ? 'status-warning'
                : 'status-info';
    ...
    formatted = formatted.replace(
      /^(✅|❌|⚠️|📊|🔍|🛠️|📋|🚀|📈|💡|🎯)\s+([^：:\n]+)[：:]\s*$/gm,
      '<div class="status-section-header"><span class="status-icon-inline">$1</span><span class="status-section-title">$2</span></div>'
    );

    // 处理状态图标后的列表内容（如 "✅ 已具备的能力:\n- item1\n- item2"）
    // 将状态区块标题和后续内容组合在一起
    formatted = formatted.replace(
      /(<div class="status-section-header">[\s\S]*?<\/div>)([\s\S]*?)(?=<div class="status-section-header">|<h[1-3]|##|###|$)/g,
