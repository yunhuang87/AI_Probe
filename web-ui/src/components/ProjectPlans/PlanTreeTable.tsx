...
// 将树形结构转换为表格数据（保留树形结构，用于Ant Design Table的expandable功能）
function convertTreeToTableData(
  nodes: PlanTreeNode[],
  level: number = 0,
  parentId?: string
): PlanTableItem[] {
  ...
  for (const node of nodes) {
    const item: PlanTableItem = {
      ...
      children: node.children && node.children.length > 0
        ? convertTreeToTableData(node.children, level + 1, node.id)
        : undefined,
    };
...
// SortableTableRow将在PlanTreeTable内部定义，以便访问flatData
...
export default function PlanTreeTable({
  ...
  const tableData = useMemo(() => {
    return convertTreeToTableData(tree, 0, undefined);
  }, [tree]);

  // 扁平化数据用于拖拽（只包含可见的节点）
  const flatData = useMemo(() => {
    const flatten = (nodes: PlanTableItem[], result: PlanTableItem[] = []): PlanTableItem[] => {
      for (const node of nodes) {
        result.push(node);
    ...
    return flatten(tableData);
  }, [tableData, expandedNodes]);

  // 创建SortableTableRow组件（需要访问flatData和activeId）
  const SortableTableRowComponent = useCallback((props: any) => {
    const recordId = props['data-row-key'];
    const actualItem = flatData.find((d) => d.id === recordId);
    ...
    const isDraggable = !actualItem.is_readonly && actualItem.type === 'task'; // 只有任务节点可拖拽
    ...
    const {
      ...
      id: recordId,
      disabled: !isDraggable, // 阶段节点和只读节点不可拖拽
    });
    ...
    const style = {
      ...
      transition,
      opacity: activeId === recordId ? 0.5 : 1,
      cursor: isDraggable ? 'grab' : 'default',
    };

    const isPhase = actualItem.type === 'phase';
    ...
    return (
      <tr
        ...
        ref={setNodeRef}
        style={style}
        className={`group hover:bg-gray-50 ${isPhase ? 'bg-blue-50/50' : ''} ${activeId === recordId ? 'shadow-lg' : ''} ${props.className || ''}`}
        {...(isDraggable ? attributes : {})} // 只有可拖拽的行才添加拖拽属性
        {...(isDraggable ? listeners : {})} // 只有可拖拽的行才添加拖拽监听器
      >
        {props.children}
      </tr>
    );
      ...
      render: (_: any, record: PlanTableItem) => {
        ...
        return (
          <div className="flex items-center gap-2">
            {/* 展开/折叠图标由Ant Design Table的expandable处理，这里不需要 */}
        ...
        <SortableContext items={flatData.map((item) => item.id)} strategy={verticalListSortingStrategy}>
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            <Table
              ...
              expandable={{
                expandedRowKeys: Array.from(expandedNodes),
                onExpandedRowsChange: (keys) => {
                  setExpandedNodes(new Set(keys as string[]));
                },
                expandIcon: ({ expanded, onExpand, record }) => {
                  const hasChildren = record.children && record.children.length > 0;
                  if (!hasChildren) return <span className="w-5 inline-block" />;
                  return (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
