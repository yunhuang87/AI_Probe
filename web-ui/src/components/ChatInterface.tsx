...
import { intelligentChat, IntelligentChatRequest } from '@/lib/api/chat';
...
export function ChatInterface({ sessionId }: ChatInterfaceProps) {
  ...
  useEffect(() => {
    ...
    if (currentSessionId) {
      try {
        ...
        if (loadedMessages.length > 0) {
          console.log(
            '[ChatInterface] Loaded messages on mount:',
          ...
          console.log(
            '[ChatInterface] No messages found in localStorage for session:',
            currentSessionId
          );
          // 检查是否有其他会话的消息
          ...
          if (allSessions.length > 0) {
            console.log(
              '[ChatInterface] Available sessions:',
              allSessions.map((s) => ({
                id: s.id,
                title: s.title,
                messageCount: getSessionMessages(s.id).length,
              }))
            );
          }
        }
      } catch (e) {
        console.error('[ChatInterface] Failed to load messages on mount:', e);
      }
    } else {
      console.log('[ChatInterface] No currentSessionId on mount');
      ...
      try {
        ...
        if (savedSessionId) {
          ...
          if (sessions.find((s) => s.id === savedSessionId)) {
            console.log('[ChatInterface] Restoring sessionId from localStorage:', savedSessionId);
            switchSession(savedSessionId);
          }
        }
      } catch (e) {
        console.warn('[ChatInterface] Failed to restore sessionId:', e);
      }
    }
  }, []); // 只在组件挂载时执行一次

  // 当currentSessionId变化时，重新加载消息
  // 注意：useChat hook 已经处理了会话变化时的消息加载，这里作为备用
  useEffect(() => {
    if (currentSessionId) {
      try {
        const loadedMessages = getSessionMessages(currentSessionId);
        ...
        if (loadedMessages.length > 0 && process.env.NODE_ENV === 'development') {
          console.log(
            '[ChatInterface] Loaded messages on session change:',
            loadedMessages.length,
            'session:',
            currentSessionId
          );
        }
        // 只在hook的消息为空时才设置（避免覆盖hook已加载的消息）
        if (messagesFromHook.length === 0 && loadedMessages.length > 0) {
          setMessages(loadedMessages);
        }
      } catch (e) {
  ...
  }, [currentSessionId, messagesFromHook.length]);
  ...
  useEffect(() => {
    setMessages((prev) => {
      ...
      if (messagesFromHook.length === 0 && prev.length > 0 && streamingIds.size === 0) {
          ...
          try {
            ...
            if (loadedMessages.length > 0) {
              ...
              if (process.env.NODE_ENV === 'development') {
                console.log(
                  '[ChatInterface] Reloaded messages from localStorage (hook empty):',
                  loadedMessages.length
                );
              }
              return loadedMessages;
        ...
        // 如果加载失败，保留本地消息（不记录日志，避免噪音）
        return prev;
      }

      if (streamingIds.size === 0) {
        // 没有流式更新，智能合并hook的消息和本地消息
        if (messagesFromHook.length > 0) {
          // 合并策略：保留本地消息中内容更长的版本（可能是流式更新中的最新内容）
          const mergedMessages = messagesFromHook.map((hookMsg) => {
            // 查找本地是否有相同ID的消息，如果有且内容更长，保留本地内容
            const localMsg = prev.find((m) => m.id === hookMsg.id);
            if (
              localMsg &&
              localMsg.content &&
              localMsg.content.length > (hookMsg.content?.length || 0)
            ) {
              // 本地消息内容更长，保留本地内容（可能是流式更新中的最新内容）
              return { ...hookMsg, content: localMsg.content, status: localMsg.status };
            }
            return hookMsg;
          });

          // 添加本地有但hook没有的消息（可能是刚创建的流式消息）
          const localOnlyMessages = prev.filter(
            (localMsg) =>
              !messagesFromHook.find((hookMsg) => hookMsg.id === localMsg.id) &&
              localMsg.content &&
              localMsg.content.trim().length > 0
          );

          return [...mergedMessages, ...localOnlyMessages];
        }

        // 如果hook的消息也为空，尝试从localStorage加载
        if (currentSessionId && prev.length === 0) {
          try {
            ...
            if (loadedMessages.length > 0) {
              ...
              if (process.env.NODE_ENV === 'development') {
                console.log(
                  '[ChatInterface] Loaded messages from localStorage (fallback):',
                  loadedMessages.length
                );
              }
              return loadedMessages;
            ...
            console.warn(
              '[ChatInterface] Failed to load messages from localStorage (fallback):',
              e
            );
          }
        }
  ...
  const [useDynamicWorkflow, setUseDynamicWorkflow] = useState(false); // 默认走 LLM，动态工作流仅在显式触发时使用
  ...
  const loadWorkflows = async () => {
    try {
      ...
      // listWorkflows 已经处理了404错误，这里只处理其他错误
      ...
      if (!is404) {
        console.error('[ChatInterface] Failed to load workflows:', error);
      }
      // 设置空列表，避免UI错误
      setWorkflows([]);
    }
    ...
    if (content.startsWith('/kb ') || content.startsWith('/knowledge ')) {
      ...
      try {
        ...
        const knowledgePrompt = [
          '你是企业知识库助手。',
          '只能基于“知识库内容”回答，禁止编造；如内容不足，请直接回答：知识库未找到相关内容。',
          '',
          `用户问题：${query}`,
          '',
          '知识库内容：',
        ...
        const llmResult = await intelligentChat(chatRequest);
        const llmOutput =
          llmResult?.output ||
          llmResult?.error ||
          '知识库检索完成，但模型未返回可用内容。';
        sendMessage(llmOutput, 'ai', 'AI助手');
      } catch (error) {
        console.error('Knowledge search failed:', error);
        sendMessage('❌ 知识库检索失败，请稍后重试。', 'system', '系统');
      }
      return;
    }
              ...
              switch (data.type) {
                ...
                case 'chunk':
                  if (data.data?.chunk) {
                    ...
                    const chunkText =
                      typeof data.data.chunk === 'string'
                        ? data.data.chunk
                        : String(data.data.chunk || '');
                    if (chunkText) {
                ...
                case 'complete':
                  ...
                  if (data.data?.result?.response) {
                    const finalResponse =
                      typeof data.data.result.response === 'string'
                        ? data.data.result.response
                        : String(data.data.result.response || '');
                    ...
                    if (output) {
                      ...
                      if (outputText && outputText.trim().length > 0) {
                        console.log('[Chat] Output found, length:', outputText.length);
                        // 检查输出是否已经在内容中（使用更宽松的检查）
                        const outputStart = outputText.substring(
                          0,
                          Math.min(100, outputText.length)
                        );
                        const outputEnd =
                          outputText.length > 100
                            ? outputText.substring(outputText.length - 100)
                            : '';
                        const alreadyIncluded =
                          outputStart &&
                  ...
                  if (
                    ...
                    (aiMessageContent === '🤔 正在分析您的请求...' ||
                      aiMessageContent.trim().length === 0)
                  ) {
                    // 如果没有任何内容，设置默认完成消息
                    if (aiMessageContent.trim().length === 0) {
                      aiMessageContent = '🔄 处理完成，但未收到响应内容';
                    } else {
                  ...
                  if (!aiMessageContent || aiMessageContent.trim().length === 0) {
                    console.warn('[Chat] Content is empty after complete, using fallback');
                    aiMessageContent =
                      '🔄 处理完成，但未收到响应内容。请检查后端服务是否正常运行。';
                  }
  ...
  const extractFinalOutputText = (value: any): string | null => {
    if (value === null || value === undefined) return null;
    if (typeof value === 'string' && value.trim().length > 0) {
      const text = value.trim();
      // Try to extract formatted_output or main_content from Python-like dict strings.
      const extracted =
    ...
    if (typeof value === 'object') {
      const formattedOutput = value.formatted_output;
      if (typeof formattedOutput === 'string' && formattedOutput.trim().length > 0) {
        return formattedOutput;
      }
      if (formattedOutput != null && String(formattedOutput).trim().length > 0) {
        return String(formattedOutput);
      }

      const finalOutput = value.final_output;
      if (typeof finalOutput === 'string' && finalOutput.trim().length > 0) {
        return finalOutput;
      }
      if (finalOutput != null && String(finalOutput).trim().length > 0) {
      ...
      if (output != null && String(output).trim().length > 0) {
        return String(output);
      }

      if (value.synthesized_result?.main_content) {
        return String(value.synthesized_result.main_content);
      }
      if (value.synthesized_result?.summary) {
        return String(value.synthesized_result.summary);
      }

      if (value.final_result) {
        const nested = extractFinalOutputText(value.final_result);
        if (nested) return nested;
      }
      if (value.result) {
        const nested = extractFinalOutputText(value.result);
        if (nested) return nested;
      }
    }
  ...
  const getFinalOutputFromChunks = (chunks: WorkflowChunk[]): string | null => {
    const lastChunk = [...chunks]
      ...
      .find(
        (chunk) =>
          ...
          chunk.result ||
          chunk.data?.result ||
          chunk.data?.final_result
      );

    if (!lastChunk) return null;

    const payload =
      lastChunk.final_result ||
      lastChunk.result ||
      lastChunk.data?.result ||
      lastChunk.data?.final_result;

    return extractFinalOutputText(payload);
  };
  ...
  const handleDynamicWorkflow = async (content: string, user: any) => {
    ...
    try {
      ...
      console.log('[DynamicWorkflow] Request context:', {
        user_input: content.substring(0, 50),
        user_id: user.user_id,
        session_id: currentSessionId,
        conversation_history_length: recentMessages.length,
      });
      ...
      try {
        console.log('[DynamicWorkflow] About to call executeDynamicWorkflow...');
        let chunkCount = 0;
        ...
        console.log('[DynamicWorkflow] Starting for-await loop...');
        ...
        for await (const chunk of executeDynamicWorkflow({
          ...
          if (!loopStarted) {
            console.log('[DynamicWorkflow] ✅ For-await loop started, received first chunk');
            loopStarted = true;
          }
          chunkCount++;
          console.log(`[DynamicWorkflow] Received chunk ${chunkCount}:`, {
            type: chunk.type,
            stage: chunk.stage,
            hasMessage: !!chunk.message,
            messageLength: chunk.message?.length || 0,
            messagePreview: chunk.message?.substring(0, 50) || 'no message',
            fullChunk: chunk,
          });
          ...
          if (chunk.type === 'thinking' && chunk.message) {
            ...
            if (chunkCount % 20 === 0 || chunkCount < 5) {
              // 每20个chunk记录一次，或前5个chunk都记录
              console.log(
                `[DynamicWorkflow] Saving thinking chunk ${chunkCount} to workflowChunks:`,
                {
                  messageLength: chunk.message.length,
                  messagePreview: chunk.message.substring(0, 50) + '...',
                  messageEnd:
                    '...' + chunk.message.substring(Math.max(0, chunk.message.length - 30)),
                }
              );
            }
          }
          ...
          if (chunk.type === 'thinking') {
            ...
            if (chunk.message) {
              ...
              // 如果内容长度显著增加（超过30个字符），或者流式输出完成，则更新
              // 降低阈值，确保更频繁地更新UI，让用户看到思考过程
              const currentThinkingMatch = messageContent.match(
                /💭 思考过程\n\n(.+?)(?:\n\n⚙️ 执行过程|$)/s
              );
              const currentThinkingLength = currentThinkingMatch
                ? currentThinkingMatch[1].length
                : 0;
              const newThinkingLength = cleanMessage.length;
              const lengthIncreased = newThinkingLength - currentThinkingLength >= 30; // 降低阈值从50到30

              // 决定是否更新UI：
              // 1. 如果流式输出完成（!isStreaming），必须更新
              // 2. 如果内容显著增加（lengthIncreased），必须更新
              // 3. 如果还没有显示思考内容（!hasExistingThinking），必须更新
              // 4. 如果是第一个thinking chunk，必须更新（确保立即显示）
              // 5. 否则，如果是流式输出且内容变化不大，可以跳过更新（减少UI刷新）
              const isFirstThinkingChunk = chunkCount === 1 || !hasExistingThinking;
              const shouldUpdateUI =
                !isStreaming || lengthIncreased || !hasExistingThinking || isFirstThinkingChunk;
              ...
              if (shouldUpdateUI) {
                ...
                // 调试：记录thinking chunk的更新
                console.log(`[DynamicWorkflow] Thinking chunk ${chunkCount} - Updating UI:`, {
                  hasExistingThinking,
                ...
                // 调试：记录跳过的更新
                console.log(
                  `[DynamicWorkflow] Thinking chunk ${chunkCount} - Skipping UI update:`,
                  {
                    isStreaming,
                    lengthIncreased,
                    currentLength: currentThinkingLength,
                    newLength: newThinkingLength,
                  }
                );
              }
            ...
            if (chunk.stage === 'layer_start') {
              ...
              console.log(
                '[DynamicWorkflow] Full chunk data:',
              ...
              if (chunk.final_result) {
                console.log(
                  '[DynamicWorkflow] final_result found in chunk:',
                  Object.keys(chunk.final_result)
                );
              } else {
            ...
            if (executionContent) {
              ...
              if (!finalThinkingContent || finalThinkingContent.trim().length === 0) {
                const thinkingMatch = messageContent.match(
                  /💭 思考过程\n\n(.+?)(?:\n\n⚙️ 执行过程|$)/s
                );
                ...
                if (existingThinkingContent && existingThinkingContent.trim().length > 0) {
                  ...
                  console.log(
                    '[DynamicWorkflow] Restored thinkingContent from messageContent:',
                    existingThinkingContent.length,
                    'chars'
                  );
                }
              }
              ...
              if (
                ...
                (chunk.stage === 'layer_start' && chunk.layer_number === 1)
              ) {
                console.log('[DynamicWorkflow] Execution started - Thinking content status:', {
                  ...
                  thinkingContentPreview: thinkingContent
                    ? thinkingContent.substring(0, 100) + '...'
                    : 'empty',
                  thinkingContentEnd: thinkingContent
                  ...
                  messageContentEnd:
                    messageContent.length > 200
                      ? '...' + messageContent.substring(messageContent.length - 100)
                      : messageContent,
                });
              }
              ...
              if (finalThinkingContent && finalThinkingContent.trim().length > 0) {
                ...
                // 如果思考内容完全丢失，记录警告
                console.warn(
                  '[DynamicWorkflow] ⚠️ Thinking content is missing when execution starts!',
                  {
            ...
            // 检查是否已经收到了有效的执行数据（非错误类型）
            const hasValidExecutionChunks = chunks.some(
              (c) => c.type === 'execution' || c.type === 'design' || c.type === 'thinking'
            );

            // 检查是否只收到了错误 chunk（没有其他有效数据）
            const onlyErrorChunks = chunks.length > 0 && chunks.every((c) => c.type === 'error');
            ...
            // 如果已经收到了有效的执行数据，且是网络错误，也忽略（可能是正常完成但连接提前关闭）
            if (hasValidExecutionChunks && isNetworkError) {
              console.warn(
                '[DynamicWorkflow] Network error after receiving valid execution chunks, ignoring for UI:',
                errorMsg,
                `(received ${chunkCount} chunks, ${chunks.length} total)`
              );
              // 不更新 messageContent，不设置 shouldUpdate，继续处理下一个chunk
              continue;
            }
            ...
            if (onlyErrorChunks && !hasValidExecutionChunks) {
              // 如果是网络错误，且响应状态可能是200（流中断），静默忽略
              // 因为可能是正常完成但连接提前关闭
              if (isNetworkError) {
                console.warn(
                  '[DynamicWorkflow] Network error chunk received as only chunk (likely connection close), ignoring:',
                  errorMsg
                );
                // 不更新 messageContent，不设置 shouldUpdate，继续处理下一个chunk
                continue;
                ...
                // 非网络错误，显示错误
                console.error(
                  '[DynamicWorkflow] Only error chunks received (non-network error), showing error:',
                  chunk
                );
              ...
              shouldUpdate = true;
              console.error('[DynamicWorkflow] Non-network error chunk received:', chunk);
            } else {
              // 网络错误但已有有效数据，忽略
              console.warn(
                '[DynamicWorkflow] Network error chunk ignored (has valid data):',
                errorMsg
              );
            }
          }
          ...
          if (shouldUpdate) {
            ...
            // 更新localStorage（每次更新都保存，确保刷新后不丢失）
            try {
              updateMessage(currentSessionId, aiMessageId, {
                content: messageContent,
                status:
              ...
              // 验证保存是否成功
              const savedMessages = getSessionMessages(currentSessionId);
              const savedMessage = savedMessages.find((m) => m.id === aiMessageId);
              if (!savedMessage || savedMessage.content !== messageContent) {
                console.warn(
                  '[DynamicWorkflow] Message update may have failed, retrying with saveMessage'
                );
                // 如果updateMessage失败，使用saveMessage重新保存
                const messageToSave = messages.find((m) => m.id === aiMessageId) || {
                  id: aiMessageId,
                  type: 'text' as const,
                  content: messageContent,
                  sender: 'AI助手',
                ...
                saveMessage(currentSessionId, { ...messageToSave, content: messageContent });
              }
            } catch (updateError) {
              console.error('[DynamicWorkflow] Failed to update message:', updateError);
              // 如果updateMessage失败，尝试使用saveMessage
              try {
                const messageToSave = messages.find((m) => m.id === aiMessageId) || {
                  id: aiMessageId,
                  type: 'text' as const,
                ...
                console.error('[DynamicWorkflow] Failed to save message as fallback:', saveError);
              }
            }
        ...
        if (chunkCount === 0) {
          console.warn(
            '[DynamicWorkflow] ⚠️ No chunks received! Stream may have been interrupted before any data arrived.'
          );
          // 更新消息，显示等待状态
          const waitingMessage = '💭 正在分析您的请求，请稍候...';
          updateMessage(currentSessionId, aiMessageId, {
          ...
          setMessages((prev) =>
            prev.map((m) =>
              m.id === aiMessageId ? { ...m, content: waitingMessage, status: 'sending' } : m
            )
          );
        }

        // 检查是否已有最终结果（即使流被中断，也可能已经收到结果）
        const hasFinalResult = chunks.some(
          (c) =>
            (c.type === 'execution' &&
              c.stage === 'execution_complete' &&
              (c.final_result || c.result)) ||
        ...
        if (!hasFinalResult && chunks.length > 0) {
          ...
          if (lastExecutionChunk?.agent_result?.output) {
            console.log(
              '[DynamicWorkflow] Found result in last execution chunk, stream may have been interrupted'
            );
          }
        }

        const finalOutputText = getFinalOutputFromChunks(chunks);
        if (finalOutputText && currentSessionId) {
          const nextContent = finalOutputText.trim();
          if (nextContent && nextContent !== messageContent) {
            ...
            setMessages((prev) =>
              prev.map((m) =>
                m.id === aiMessageId ? { ...m, content: messageContent, status: 'sent' } : m
              )
            );
          }
        }

        // 确保最终消息已保存（以当前 messageContent 为准，避免状态滞后）
        if (currentSessionId) {
          const persistedContent = messageContent && messageContent.trim().length > 0
            ? messageContent
            : '✅ 执行完成';
          const finalMessageToPersist: Message = {
            id: aiMessageId,
            type: 'text',
            ...
            metadata: {
              workflowMessageId,
              workflowType: 'dynamic',
              finalOutput: persistedContent,
            },
          };

          try {
            try {
              localStorage.setItem(`chat_ai_reply_${aiMessageId}`, persistedContent);
            } catch (e) {
              console.warn('[DynamicWorkflow] Failed to persist AI reply backup:', e);
            }
            saveMessage(currentSessionId, finalMessageToPersist);
            // 验证保存是否成功
            const savedMessages = getSessionMessages(currentSessionId);
            const savedMessage = savedMessages.find((m) => m.id === aiMessageId);
            if (!savedMessage) {
              console.error('[DynamicWorkflow] Final message was not saved! Retrying...');
              saveMessage(currentSessionId, finalMessageToPersist);
            } else if (savedMessage.content !== finalMessageToPersist.content) {
              console.warn('[DynamicWorkflow] Final message content mismatch, updating...');
              updateMessage(currentSessionId, aiMessageId, {
                content: finalMessageToPersist.content,
              });
            } else {
              console.log('[DynamicWorkflow] Final message saved successfully:', {
                messageId: aiMessageId,
                contentLength: finalMessageToPersist.content.length,
                sessionId: currentSessionId,
              });
            }
          } catch (saveError) {
            console.error('[DynamicWorkflow] Failed to save final message:', saveError);
            try {
              const messages = getSessionMessages(currentSessionId);
              const index = messages.findIndex((m) => m.id === aiMessageId);
              if (index >= 0) {
                messages[index] = finalMessageToPersist;
              } else {
                messages.push(finalMessageToPersist);
              }
              localStorage.setItem(`chat_messages_${currentSessionId}`, JSON.stringify(messages));
            } catch (lastError) {
              console.error('[DynamicWorkflow] Last resort save failed:', lastError);
            }
          }
        }
        ...
        if (currentSessionId) {
          try {
            const loadedMessages = getSessionMessages(currentSessionId);
            console.log(
              '[DynamicWorkflow] Reloaded messages after completion:',
              loadedMessages.length
            );
            setMessages(loadedMessages);
          } catch (loadError) {
            console.error('[DynamicWorkflow] Failed to reload messages:', loadError);
          }
        }
      } catch (error: any) {
        ...
        if (hasFinalResult) {
          console.log(
            '[DynamicWorkflow] Error occurred but final result was already received, will be displayed'
          );
