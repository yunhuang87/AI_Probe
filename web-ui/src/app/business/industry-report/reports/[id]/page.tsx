...
import { getReport, Report } from '@/lib/api/industryReport';

const API_BASE =
  process.env.NEXT_PUBLIC_INDUSTRY_REPORT_API_BASE || 'http://localhost:8021/api';

export default function IndustryReportDetailPage() {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/reports/download/${report.file_path}`);
      if (!response.ok) throw new Error('下载失败');
      const blob = await response.blob();

      // 支持 File System Access API 时，优先使用
      // @ts-ignore
  ...
  if (!report) {
    return <div className="p-8 text-center text-red-600">未找到报告</div>;
  }
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 gap-8">
        <div className="bg-white p-6 rounded-lg shadow border">
          ...
          <div
            className="overflow-x-auto excel-preview"
