...
import { uploadFile, generateReport, getReport, Report } from '@/lib/api/industryReport';

export default function IndustryReportUploadPage() {
      ...
      const poll = setInterval(async () => {
        try {
          ...
          if (report.status === 'completed') {
            clearInterval(poll);
            setGenerating(false);
            router.push(`/business/industry-report/reports/${report.id}`);
          } else if (report.status === 'failed') {
            clearInterval(poll);
            setGenerating(false);
            alert('生成报告失败，请检查行业报告服务日志');
          } else {
            setProgress({
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold mb-2">行业产品报告自动化</h1>
      <p className="text-gray-600 mb-4">
        上传运营数据，调用大模型自动生成行业产品分析报告，并支持在线预览与下载。
      </p>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border space-y-2">
        <h2 className="text-lg font-semibold">分析提示词（可选）</h2>
      ...
      <form className="flex flex-col items-end" onSubmit={handleGenerate}>
        {generating && progress && (
