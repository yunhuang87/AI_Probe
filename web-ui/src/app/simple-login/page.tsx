...
// 与其它页面一致：优先走 API 网关（8080），再回退到直连 auth
const getLoginBaseUrl = () => {
  const gateway = process.env.NEXT_PUBLIC_API_GATEWAY_URL || '';
  if (gateway) return gateway.replace(/\/+$/, '') + '/api/auth';
  return process.env.NEXT_PUBLIC_AUTH_SERVICE_URL || 'http://localhost:8080/api/auth';
};
