...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const url = `${getBaseUrl()}/knowledge-bases/${id}/documents?page=${page}&page_size=${pageSize}`;
    ...
    if (!response.ok) {
      const error = await response
        .json()
        .catch(() => ({ detail: 'Failed to fetch knowledge base documents' }));
      return NextResponse.json(
        { error: error.detail || 'Failed to fetch knowledge base documents' },
