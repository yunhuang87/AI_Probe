    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">管理项目任务和进度跟踪</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-16 text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">开始创建您的第一个任务</p>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div className="flex-1 space-y-3">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span
                      className={`px-2.5 py-1 text-xs font-semibold rounded-full border flex items-center gap-1.5 shadow-sm ${getStatusColor(task.status)}`}
                    >
                  ...
                  {task.description && (
                    <p className="text-sm text-gray-600 line-clamp-2 pl-1">{task.description}</p>
                  )}

                  <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 pl-1">
                    {task.project_name && (
                      <span className="flex items-center gap-1">
                        <span className="font-medium">项目:</span> {task.project_name}
                      </span>
                    )}
                    {task.assigned_to && (
                      <span className="flex items-center gap-1">
                        <span className="font-medium">负责人:</span> {task.assigned_to}
                      </span>
                    )}
                    {task.due_date && (
                      <span className="flex items-center gap-1">
