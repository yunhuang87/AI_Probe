...
try {
    ...
    $uri = "$baseUrl/data-assets?$queryString"
...
# 6. 验证数据库字段
Write-Host ""
Write-Host "[8/8] 验证数据库字段..." -ForegroundColor Yellow
try {
    $sql = "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'data_assets' AND column_name IN ('classification_dimensions', 'standardized_tags');"
