# 完整部署：重建数据库并同步迁移
# 使用方法: .\scripts\deploy-rebuild-db-final.ps1
...
# 步骤1: 检查SSH连接
Write-Host "[1/7] 检查SSH连接..." -ForegroundColor Yellow
try {
...
# 步骤2: 停止服务
Write-Host ""
Write-Host "[2/7] 停止服务..." -ForegroundColor Yellow
...
# 步骤3: 备份并重建数据库
Write-Host ""
Write-Host "[3/7] 备份并重建数据库..." -ForegroundColor Yellow
Write-Host "警告：这将删除所有现有数据！" -ForegroundColor Red
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
ssh "${ServerUser}@${ServerIP}" "mkdir -p /backup/database" | Out-Null
ssh "${ServerUser}@${ServerIP}" "docker exec enterprise-ai-postgres pg_dump -U ai_user -d ai_platform -F c -f /tmp/backup_$timestamp.dump" | Out-Null
ssh "${ServerUser}@${ServerIP}" "docker cp enterprise-ai-postgres:/tmp/backup_$timestamp.dump /backup/database/" | Out-Null
...
# 步骤4: 上传所有迁移文件
Write-Host ""
Write-Host "[4/7] 上传所有迁移文件..." -ForegroundColor Yellow
...
# 步骤5: 执行所有迁移
Write-Host ""
Write-Host "[5/7] 执行数据库迁移（从头开始）..." -ForegroundColor Yellow
...
# 步骤6: 验证版本
Write-Host ""
Write-Host "[6/7] 验证迁移版本..." -ForegroundColor Yellow
